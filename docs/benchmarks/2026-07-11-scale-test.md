...
## 追记(2026-07-12):查询特征路由(Phase 6)

针对结论 3 实现 df 自适应路由:查询含"稀有但在库"词元(`0 < df ≤ max(3, ⌈0.5%·N⌉)`)才启用 BM25 通道。路由后复测:

| 语料 | 混合 MRR(路由前) | 混合 MRR(路由后) | 对照纯向量 |
|---|---|---|---|
| 咖啡馆 16 块 | 0.955 | 0.955(不回归) | > 0.939 ✓ |
| MDN 21,838 块 | 0.922 | **0.933** | = 0.933 ✓ |

混合检索从"随语料可能帮倒忙"变为**处处不劣于纯向量**:稀有词查询保留词面兜底优势,常见词查询零噪声零 BM25 开销。附带修复:分词过滤纯标点词元(标点曾作为伪"稀有词"误触发路由)。
